...
export default function TerminPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      setUserId(uid)
      setUsername(session.user.user_metadata?.username || session.user.email?.split('@')[0] || 'FahrschÃ¼ler')
      setLoading(false)
    })
  }, [router])
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ textAlign: 'center', maxWidth: '400px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
          Du bist noch nicht als FahrschÃ¼ler freigeschaltet. Dein Fahrlehrer Tolga wird dich bald freischalten.
        </p>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
            WÃ¤hle einen freien Termin fÃ¼r deine Fahrstunde. Montag bis Samstag, 12:00â€“22:00 Uhr Â· Niedersachsen 2026
          </p>
        </div>

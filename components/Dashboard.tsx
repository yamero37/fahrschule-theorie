...
function buildWhatsappUrl(phone: string, name: string, date: string, startTime: string, durMin: number): string {
  const d    = new Date(date + 'T12:00:00')
  const end  = startTime.slice(0,5).split(':').map(Number)
  const endMin = end[0] * 60 + end[1] + durMin
  const endStr = `${String(Math.floor(endMin / 60)).padStart(2,'0')}:${String(endMin % 60).padStart(2,'0')}`
  const dateStr = `${DAYS_LONG[d.getDay()]}, ${d.getDate()}. ${MONTHS_SHORT[d.getMonth()]} ${d.getFullYear()}`

  const msg = `Hallo ${name}! 👋\n\nDein Termin bei TolDrive wurde bestätigt ✅\n\n📅 Datum: ${dateStr}\n🕐 Uhrzeit: ${startTime.slice(0,5)} – ${endStr} Uhr\n\nBis dann,\nDein Fahrlehrer Tolga 🚗`
  return `https://wa.me/${normalizeWaPhone(phone)}?text=${encodeURIComponent(msg)}`
}
            ...
            return (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', padding: '.85rem 1.1rem', borderRadius: 12, background: a.status === 'pending' ? 'rgba(var(--gold-rgb),.04)' : 'var(--surface-2)', border: a.status === 'pending' ? '1px solid rgba(var(--gold-rgb),.15)' : '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                  ...
                  {a.status === 'accepted' && a.phone && (
                    <a
                      href={buildWhatsappUrl(a.phone, a.full_name ?? a.student_name, a.date, a.start_time, a.duration_min)}
                      target="_blank" rel="noopener noreferrer"
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 11px', borderRadius: 7, fontSize: '.68rem', fontWeight: 700, background: 'rgba(37,211,102,.12)', border: '1px solid rgba(37,211,102,.35)', color: '#25d366', textDecoration: 'none', whiteSpace: 'nowrap' }}
                    >
